module.exports = class TimerManager {
        ...
        async setTimer(channelID, time, userID, title, updateRedis = true) {
                ...
                const timeout = setTimeout(async () => {
                        try {
                                ...
                                await channel.send(
                                        `<@${userID}>, you wanted me to remind you of: **"${title}"**.`
                                );
                        } finally {
                ...
                if (updateRedis)
                        await this.client.redis.hset('timer', {
                                [`${channelID}-${userID}`]: JSON.stringify(
                                        data
                                ),
                        });
        ...
        exists(channelID, userID) {
                return this.client.redis.hexists(
                        'timer',
                        `${channelID}-${userID}`
                );
        }
